## Table of Contents
- [Docker Engine Fundamentals](#docker-engine-fundamentals)
  - [pull image from hub.docker.com (pulls latest version)](#pull-image-from-hubdockercom-pulls-latest-version)
  - [pull specific image version](#pull-specific-image-version)
  - [pull and run in 1 command](#pull-and-run-in-1-command)
  - [view current images on disk](#view-current-images-on-disk)
  - [run image -i (interactive) -t (terminal)](#run-image--i-interactive--t-terminal)
  - [run image in -d (detached) mode](#run-image-in--d-detached-mode)
  - [run image with specified name](#run-image-with-specified-name)
  - [view docker running process (notice id of -d container)](#view-docker-running-process-notice-id-of--d-container)
  - [view docker processes including those that have exited -a (all)](#view-docker-processes-including-those-that-have-exited--a-all)
  - [restart container - use container ID (stop / start)](#restart-container---use-container-id-stop--start)
  - [bind different host port to container port](#bind-different-host-port-to-container-port)
...
- [tutorial project steps](#tutorial-project-steps)
  ...
  - [2- create docker network](#2--create-docker-network)
  - [find out exposed port of image](#find-out-exposed-port-of-image)
  - [3- run mongo image with init user and pass over ride, network, name, detached](#3--run-mongo-image-with-init-user-and-pass-over-ride-network-name-detached)
  - [4- run mongo express with admin user, admin pass, network, name, server name](#4--run-mongo-express-with-admin-user-admin-pass-network-name-server-name)
  - [remove docker containers](#remove-docker-containers)
  - [WARNING Memory overcommit must be enabled!](#warning-memory-overcommit-must-be-enabled)
  - [5- use mongo:4.4 and reuse mongo-express](#5--use-mongo44-and-reuse-mongo-express)
  - [6- Use docker-compose to run a .yaml file](#6--use-docker-compose-to-run-a-yaml-file)
  - [7- use docker-compose to stop containers](#7--use-docker-compose-to-stop-containers)
- [tutorial build docker file (DEMO)](#tutorial-build-docker-file-demo)
  ...
  - [make docker use /home to store all data](#make-docker-use-home-to-store-all-data)
